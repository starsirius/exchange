...
describe Api::GraphqlController, type: :request do
  ...
  describe 'make offer happy path' do
    ...
    it 'succeeds the process of buyer create -> add initial offer -> set shipping -> set payment -> submit -> seller accepts' do
      ...
      # Buyer adds initial offer to order
      expect do
      ...
      # Seller counters
      expect do
        seller_client.execute(OfferQueryHelper::SELLER_COUNTER_OFFER, input: { offerId: offer.id.to_s, amountCents: 600_00 })
      ...
      offer = Offer.order(created_at: :desc).first
      expect(offer).to have_attributes(
        order_id: order.id,
        amount_cents: 600_00,
        shipping_total_cents: 200_00,
      ...
      # Buyer accepts offer
      expect do
        buyer_client.execute(OfferQueryHelper::BUYER_ACCEPT_OFFER, input: { offerId: offer.id.to_s })
      end.to change(order.transactions, :count).by(1)
